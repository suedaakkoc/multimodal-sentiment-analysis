...
class AudioSentimentDatasetWav2Vec2(Dataset):
    def __init__(self, csv_path, audio_dir, processor=None, augment=False):
        try:
    ...
    def speech_file_to_array_fn(self, path):
        ...
        try:
            ...
            return np.zeros(int(TARGET_SAMPLE_RATE * MAX_AUDIO_LENGTH))
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.processor is not None:
                ...
                return {
                    "input_values": torch.tensor(speech_array),
                    "labels": label
                }
            ...
            # Return dummy data
            silent_audio = np.zeros(int(TARGET_SAMPLE_RATE * MAX_AUDIO_LENGTH))
            if self.processor is not None:
                result = self.processor(
                    silent_audio,
                    sampling_rate=TARGET_SAMPLE_RATE,
                ...
                return {
                    "input_values": torch.tensor(silent_audio),
                    "labels": 1
                }
...
# Constants for original preprocessing
SAMPLE_RATE = 22050
N_FFT = 1024
HOP_LENGTH = 512
N_MELS = 64

# Audio Transformation Pipeline (original)
